...
func TestParseMultiplicationCalls(t *testing.T) {
	tests := []struct {
		...
		{"{!how()'&where()select()@]how()}mul(884,758);-mul(971,475)who()~from()]~mul(358,23)}", []MultiplicationOperands{
			{left: 884, right: 758},
			{left: 971, right: 475},
			{left: 358, right: 23},
		}},
		{"mul(1,2)do()mul(2,3)don't()mul(3,4)do()mul(5,6)", []MultiplicationOperands{
			{left: 1, right: 2},
			{left: 2, right: 3},
			{left: 5, right: 6},
		}},
		{"xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))", []MultiplicationOperands{
			{left: 2, right: 4},
			{left: 8, right: 5},
		}},
	}
